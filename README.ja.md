...
> Podman はデーモンレスのコンテナエンジンで、Docker の代替として使用できます。以下は独立した Podman デプロイ方法です — Docker のインストールは不要です。
...
データはホストの `/home/ubuntu/data/one-api` ディレクトリに保存されます。このディレクトリが存在し、書き込み権限があることを確認するか、適切なディレクトリに変更してください。

> 注意：Podman で Docker Hub イメージをプルするには、明示的な `docker.io/` プレフィックスが必要です。

#### ローカル Dockerfile からのビルド
...
ローカルでビルドしたイメージは `localhost/one-api:latest` として参照します。

#### Podman Pod を使用したフルスタック（MySQL + Redis）

単一の Pod で全サービスを実行し、コンテナ間は `localhost` で通信します：
