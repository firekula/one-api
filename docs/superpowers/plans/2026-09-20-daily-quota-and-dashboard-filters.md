...
**结果：构建与 embed 通过；逐主题联调 15 项全部 PASS（其中 default 与 berry 的日志页各有 1 处"控件显示空白"缺口；air 的用户列表无落盘证据，记为代码核实）；跨任务 a–f 中 a/c/e 三项 FAIL（真实缺陷）、b/d/f 三项 PASS（b 的渲染优雅但数值不实、f 只到 HTTP 边界）。无产品代码改动。** 完整证据（每条命令、控件显示文本、请求 URL 与响应体、截图清单、失败注入方法）见 `.superpowers/sdd/2026-09-20-daily-quota-and-dashboard-filters/task-D7-report.md`。
...
- **逐主题 5 项联调**：三主题的 **总览 / 用户编辑 / 运营设置 / 用户列表 / 日志页全部 PASS**。要点：默认区间都是最近 7 天（`start_timestamp=1789401600&end_timestamp=1790006399`）；管理员可切「全站/仅自己」且数字随之变化（default：self `gpt-4 4/9600` → all `gpt-3.5-turbo 8/400 + gpt-4 4/9600`；berry 卡：`4/$0.019/480` → `12/$0.020/1440`）；天/小时粒度、用户/令牌/模型三个筛选在请求参数与响应体中逐条核实；**总览与日志页同窗口数字一致**（今日仅自己 = 4 次 / 9600 额度 / 480 tokens，总览与 `/api/log/stat?type=2&username=root` 双向对上）。用户编辑三态往返（default 与 air 走页面、berry 走弹窗）都做了"保存→重开读回"闭环，并单独证明 **"保存「跟随全局默认」后原值真的归零"**（PUT 里 `daily_token_limit` 由 1000 变 `0`，DB 直读为 0）。运营设置两个全局默认保存成功、**刷新后读回 1234/5678**，测后复原。用户列表三主题都在同一列给出 token 与额度两对（`960 / 不限`、`$0.00 / $0.00`），`0` 上限渲染为「不限」——**其中 air 的此项没有落盘证据（无截图、无逐格读数 JSON），记为"代码核实"而非可复核的浏览器证据**：`web/air/src/components/UsersTable.js:46-56` 与 berry 已截图的 `TableRow.js:106-117` 为同构实现（同一 `effective_daily_token_limit > 0 ? ' / ' + renderNumber(...) : ' / 不限'` 分支）。日志页三个下拉都含「全部」且选后列表/统计变化（default 11→5 行、`/api/log/stat` 给 9600；非管理员仅 token/model/type）。
- **跨任务 a–f**：
  - **a FAIL（default 与 berry 都复现）**：先选模型再让候选收窄（切用户），请求仍带 `model_name=gpt-4`（berry 实测 `…username=dailytest&model_name=gpt-4`）而**控件显示空白**（`displayOf('模型').shown === ""`），即"筛选在生效却看不见"。default 与 berry 同形（截图 `default-3a-model-blank.png`、`berry-3a-model-blank.png`）；air 因 Semi 的 `_notExist` 合成 + `withSelected` **不存在**该陷阱。**D9 已修复（commit `2f163e1`）**：两个总览各自补上 `withSelected` 把选中值前置进选项，并让 scope/username 变化同时清空 `token_name` 与 `model_name`，浏览器复测见 `.superpowers/sdd/2026-09-20-daily-quota-and-dashboard-filters/task-D9-report.md` 缺陷 (a)。
  - **b PASS（渲染优雅，数值不实）**：default 同一用户搜索行显示 `0 / 不限` 与 `$0.00 / 不限`，而分页行是 `960 / 不限` 与 `$0.00 / $0.00` —— 无 NaN/空白，但"今日 0""上限不限"是**错误的正面断言**；根因是 `/api/user/search` 不返回那四个字段（已知后端缺口）。**本项的落盘证据只有截图** `shots/default-user-search.png`（搜索框 `dailytest`、表内只剩 1 行、两格正是 `0 / 不限` 与 `$0.00 / 不限`）；同轮成功那次读到的响应体（`GET /api/user/search?keyword=dailytest` 只含 `daily_*_limit`、无那四个字段）**只存在于会话输出、未落盘**（`out-default-ops.json` 里那条 `F-search-*` 是第一次 `keydown Enter` 未提交的**失败尝试**：`reqs: []`、表仍三行，**不是**本项证据），"字段缺失"这层由后端代码（`model.SearchUsers` 不 select 该四字段）与会话读数共同佐证。berry/air 的搜索路径未单独实测。
  - **c FAIL（berry）**：brief 记在 air 名下的"三张头部卡"实测属 berry（air 总览只有两张 VChart 图）。窗口 2026/08/01–08/31（今天在窗口外）+ 小时粒度时，后端返回 `{"data":null}`，页面卡片仍是上一个窗口的 `12 / $0.020 / 1440`，**未按代码注释给 `'-'`** —— 根因 `web/berry/src/views/Dashboard/index.js` 的 `if (data) {…}` 把"成功但空"和"失败"一起当保留旧值处理。另：小时粒度且含今天时卡片给的是**当天所有小时箱合计**（正确）。**D9 已修复（commit `2f163e1`）**：`SearchLogsByDayAndModel` 改返回空切片（空区间不再序列化成 `null`），berry 侧再按 `Array.isArray(data) ? data : []` 兜底，复测空区间卡片显示 `-` 且响应体为 `{"data":[]}`，见同报告缺陷 (c)。
  ...
  - **e FAIL（air）**：`setBlockedURLs` 注入失败后，"已有数据再查询"会弹「错误：Network Error」（自动消失）并保留旧图（可接受）；**首次加载即失败**时页面**常驻**显示「**所选区间内暂无数据**」，即把"请求失败"持续呈现为"这个区间没有数据"（误导）。**更正**：第一版此处写成"无任何错误提示"是**过度断言** —— 补做的定时重测（`out-air-3e-retest.json`）表明失败瞬间 toast「错误：Network Error」**确实出现**，只是约 3s 后自动关闭（Semi `toastConstants.ERROR_TIMEOUT`），用户回头看时只剩那条常驻空态（~1.2s 读到 `{toasts:["错误：Network Error"], emptyState:true}`，~7s 后读到 `{toasts:[], emptyState:true}`）；代码路径一致（网络错误的 `error.response.status === 0` → air `showError` 的 `switch` 落 `default` → `Toast.error('错误：' + error.message)`）。故该缺陷的定性是**常驻的误导信息**，不是"什么也不提示"；归属文件仍为 `web/air/src/pages/Dashboard/index.js`（`catch` 吞错、`finally` 仅结束 loading、`hasData` 保持 false）。真实的越界区间被前端守卫拦下并提示「错误：小时粒度最多查询 90 天」，这条是诚实的。**D9 已修复（commit `2f163e1`）**：新增 `loadError` 标记，失败时渲染「数据加载失败，图表可能不是当前筛选条件的结果」并给「重试」，真正为空的区间仍显示原空态（复测四种场景，见同报告缺陷 (e)）。
...
## 合并说明与后续项（最终全分支审查的收尾记录）

**最终全分支审查结论：With fixes → 已修复，可合并。** 审查覆盖 42 个提交、54 个文件（+4062/−598），
认定后端正确且自洽（三态重置、判定先于预扣、角色作用域含伪造用例、每页一次查询），三个浏览器发现的
缺陷也修在了正确的层。修复波（`8e862f5`、`7fbbabb`、`654ffe9`）闭合了六项 Important：

1. **三个主题的总览都没有实现"管理员默认看全站"** —— 设计文档的决议表写的是「管理员默认看全站，页面内可切回
   「仅自己」」，而实现全部初始化为 `self`，等于本次需求要改变的现状仍未改变，且管理员专用的"用户"下拉在切到
   全站前不可达。已改为 `isAdminUser ? 'all' : 'self'`（三主题各一行，非管理员仍强制 `self`），并做了浏览器验证。
2. **default 与 berry 会静默丢弃不在本地网格内的桶** —— 前端按浏览器本地日历生成标签、后端按服务器本地时间
   分桶，UTC 服务器 + UTC+8 浏览器是常见部署，小时粒度下整条轴都会偏移并被丢弃（air 因 D4 的修复已做并集）。
   两个主题改为并入回传标签（去重 + 排序），并在用户手册补了浏览器时区一致性的说明。
3. **`/api/user/search` 的行会显示"0 / 不限"这一错误断言** —— `SearchUsers` 返回裸 `model.User`，缺那四个
   字段。已在控制器抽出与 `GetAllUsers` 共用的投影助手（未给 `model.User` 加字段，备份导出格式因此未变），
   一处修复让三个主题同时正确。浏览器验证：搜索行从 `0 / 不限` 变为 `960 / 不限`。
4. **两处承重守卫在 CI 里不可能失败** —— (a) 本地日边界的回归测试在 `TZ=UTC` 下与源码同义，等于空转；已在
   Go 与进程两侧同时钉住非 UTC 时区（并加了偏移自检，使平台不支持时**显式变红**而非静默通过），并用变异
   检查证明去掉 `'localtime'` 后测试会失败。(b) 总览缺少 200 路径的授权测试，把 `UserId` 改回无条件置 0
   不会让任何测试变红；已补测试并同样做了变异检查。
5. **berry 的两个全局默认项标签写成"新用户单日上限"** —— 它是全站默认（对所有自身字段为 0 的用户生效），
   不是新用户设置；已改为与另两个主题一致的「（全局默认）」。
6. **air 的"查询"按钮把点击事件当作日志类型发出**（`type=[object Object]`，静默丢弃类型筛选）—— 本分支
   重建了该表单并新增了三个筛选，故一并修掉；已浏览器验证为 `type=2` 且不再整页刷新。

### 后续项（审查已分诊，建议单独任务/后续迭代，不阻塞合并）

- **前端统一的"无请求序号/取消"（C1、C5、D1、D3、D6、air 总览）**：一个决策而非六件事。最坏情况是旧响应
  覆盖较新的**视图**，而请求本身总带最新筛选、控件始终可见；真正会误导用户的"陈旧状态"类（D9 那批）已修。
- **三态编码/解码与用户列表投影在六个地方各写一遍**（三主题 × 两处，无编译器保护）：建议抽一份契约段落
  （0/-1/≥1、由未钳制的原始值推导模式、跟随全局必须显式发 0），或抽共用助手。
- **C2 的"加载失败后保存会清掉已存豁免"**：真实但低概率。修法是加 `loaded` 标记，载入失败时禁用保存，
  三主题都有该形态。
...
- `docs/getting-started/user-manual.md` 的 `### 模型倍率` 表与出厂默认不一致：`gpt-4o` 写 5（实际 2.5）、
  `gpt-4o-mini` 写 0.25（实际 0.075）、`gpt-3.5-turbo` 写 0.5（实际 0.25）。
- 同一手册的"Web 管理界面概览"表把总览页称作"首页"，而 default 主题的真实导航标签是"总览"。
