...
12. 支持**用户邀请奖励**。
13. 支持以美元为单位显示额度。
14. 支持**国际化（i18n）**，后端错误消息根据请求语言自动翻译。
15. 支持发布公告，设置充值链接，设置新用户初始额度。
16. 支持模型映射，重定向用户的请求模型，如无必要请不要设置，设置之后会导致请求体被重新构造而非直接透传，会导致部分还未正式支持的字段无法传递成功。
17. 支持失败自动重试。
18. 支持绘图接口。
19. 支持 [Cloudflare AI Gateway](https://developers.cloudflare.com/ai-gateway/providers/openai/)，渠道设置的代理部分填写 `https://gateway.ai.cloudflare.com/v1/ACCOUNT_TAG/GATEWAY/openai` 即可。
20. 支持 [Anthropic Messages API](https://docs.anthropic.com/en/api/messages) **原生入站中继**，无需格式转换即可接入 Claude Code 等原生 Anthropic 客户端。入站请求使用 `x-api-key` 认证，响应原样透传（支持 DeepSeek 等 Anthropic 兼容端点）。
21. 支持丰富的**自定义**设置，
    1. 支持自定义系统名称，logo 以及页脚。
    2. 支持自定义首页和关于页面，可以选择使用 HTML & Markdown 代码进行自定义，或者使用一个单独的网页通过 iframe 嵌入。
22. 支持通过系统访问令牌调用管理 API，进而**在无需二开的情况下扩展和自定义** One API 的功能，详情请参考此处 [API 文档](./docs/API.md)。
23. 支持 Cloudflare Turnstile 用户校验。
24. 支持用户管理，支持**多种用户登录注册方式**：
    ...
    - 支持 [OIDC 授权登录](https://openid.net/connect/)，可接入任意兼容 OIDC 协议的身份提供者。
    - 微信公众号授权（需要额外部署 [WeChat Server](https://github.com/songquanpeng/wechat-server)）。
25. 支持主题切换，设置环境变量 `THEME` 即可，默认为 `default`，欢迎 PR 更多主题，具体参考[此处](./web/README.md)。
26. 配合 [Message Pusher](https://github.com/songquanpeng/message-pusher) 可将报警信息推送到多种 App 上。
27. 支持**单日用量上限**，分别按 token 与额度两个维度限制每个用户当天的用量，可为单个用户设置豁免或独立上限，管理员可随时重置某用户的当日用量，详见[单日用量上限](#单日用量上限)。
28. 支持**总览的时间段选择**（不再局限于最近 7 天），并可按用户 / 令牌 / 模型筛选；管理员默认查看全站消耗，也可切换为仅看自己。
29. 支持**日志按用户 / 令牌 / 模型筛选**，筛选项直接取自当前已有的候选值。
...
# 例如：
docker run --name one-api -d --restart always -p 3000:3000 -e SQL_DSN="root:123456@tcp(localhost:3306)/oneapi" -e TZ=Asia/Shanghai -v /home/ubuntu/data/one-api:/data firekula/one-api
```
...
> Podman 是无守护进程的容器引擎，可替代 Docker。以下为独立的 Podman 部署方式，无需安装 Docker。
...
> 注意：Podman 拉取 Docker Hub 镜像时需要显式指定 `docker.io/` 前缀。
...
本地构建的镜像引用名为 `localhost/one-api:latest`。

#### 使用 Podman Pod 部署完整栈（MySQL + Redis）

在同一 Pod 中运行所有服务，Pod 内的容器通过 `localhost` 互相访问：
...
更完整的功能说明（渠道 / 令牌 / 用户管理、额度规则、单日用量上限、总览与日志筛选等）见[用户手册](./docs/getting-started/user-manual.md)。
...
除账户总额度外，还可以限制**每个用户当天**的用量，避免单个账号（或泄露出去的令牌）在一天之内耗尽整站额度。

- **两个维度独立生效**：单日 token 上限按 `prompt_tokens + completion_tokens` 计，单日额度上限按额度计，任一维度超限都会拦截请求（`403`，错误码 `insufficient_user_daily_tokens` / `insufficient_user_daily_quota`）。
- **判定用的是预估值**：`已用量 + prompt + 客户端传来的 max_tokens > 上限` 就会被拒。所以上限必须大于「单次请求的 prompt + max_tokens」，否则每次请求都会失败，与当天已经用了多少无关；报错消息会同时给出已用量、本次预估值和上限，据此即可判断属于哪一种情况。
- **按自然日重置**（服务器本地时区）。管理员可在`用户列表`中直接看到「今日 token / 今日额度 / 生效上限」，并用操作列的`重置当日用量`把某个用户的当天用量清零。

配置位置：全局默认在`系统设置 → 运营设置`（`单日 Token 上限（全局默认）`、`单日额度上限（全局默认）`）；单个用户在`用户管理 → 编辑`里设置，可选择`跟随全局`、`豁免（不限）`或`自定义`。更多细节见[用户手册](./docs/getting-started/user-manual.md#单日用量上限)。
...
21. `SQLITE_PATH`：自定义 SQLite 数据库文件路径，默认为工作目录下的 `one-api.db`。
...
29. `INITIAL_ROOT_ACCESS_TOKEN`：如果设置了该值，则在系统首次启动时会自动创建一个值为该环境变量的 root 用户创建系统管理令牌。
30. `ENFORCE_INCLUDE_USAGE`：是否强制在 stream 模型下返回 usage，默认不开启，可选值为 `true` 和 `false`。
31. `TEST_PROMPT`：测试模型时的用户 prompt，默认为 `Output only your specific model name with no additional text.`。
32. `ONLY_ONE_LOG_FILE`：是否只使用一个日志文件而不按日期分割，默认为 `false`。
...
1. 额度是什么？怎么计算的？One API 的额度计算有问题？
   ...
   - 默认倍率表会随版本更新（当前基线取自新版 one-api 的默认表）。**表中没有的模型一律按 1 倍计费**，不会再被高价兜底，同时日志里会记一条 `model ratio not found: <模型名>`；自建或转售的模型名不在表中时，请到`系统设置 → 模型倍率`补上，否则它的计费只会按最基础的价格计算。另外注意，一旦你在系统设置里保存过倍率表，升级时只会为缺失的模型补默认值，**不会覆盖你保存过的旧值**；想让新版本的基线完全生效，把该字段清空为 `{}` 后保存并重启即可。
2. 账户额度足够为什么提示额度不足？
