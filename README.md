...
19. 支持 [Anthropic Messages API](https://docs.anthropic.com/en/api/messages) **原生入站中继**，无需格式转换即可接入 Claude Code 等原生 Anthropic 客户端。入站请求使用 `x-api-key` 认证，响应原样透传（支持 DeepSeek 等 Anthropic 兼容端点）。
20. 支持丰富的**自定义**设置，
